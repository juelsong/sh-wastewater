...
export const ListMixin = {
    ...
    methods: {
        loadData () {
            ...
            let filter = this.buildFilterStr == undefined ? undefined : this.buildFilterStr();
        ...
        onSortChange (sortArgs) {
            if (sortArgs && sortArgs.prop) {
                this.query.$orderby = `${sortArgs.prop} ${sortArgs.order == "descending" ? "desc" : "asc"
                    }`;
            } else {
        ...
        async deletCore (ids) {
            if (await this.$confirm(this.$t("confirm.batchDelete", [`${ids.length}`]))) {
